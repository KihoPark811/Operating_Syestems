...
- 버스는 단순 선이 아닌, 데이터 전송을 위한 신호들의 묶음이다.
  ### 세 가지 구성 요소
  - 주소 버스(Address Bus): 어디(메모리나 장치)에 접근할 것인지 주소를 지정한다.
...
- 각종 하드웨어 장치를 제어하는 기능을 구현하는데 있어서 부분적으로 고급 언어로는 표현이 불가능한 내용들이 있고, 이러한 부분은 어셈블리 언어로 표현하여야 한다.
...
- **CPU 내부에 있는 아주 빠른 임시 저장 공간으로, CPU가 계산이나 명령어 수행을 할 때, 가장 빠르게 접근할 수 있는 기억 장소이다.**
- 레지스터를 사용하는 이유는 CPU가 연산을 할 때, 데이터를 바로 메모리(RAM)에서 읽고 쓰면 너무 느리기 때문이다.
  보통, 변수를 저장하거나 사칙 연산 같은 간단한 계산 또는 명령어 수행을 위한 주소 기억 등에 사용된다.
### 대표적인 레지스터 종류(x86 기준)
![image](https://github.com/user-attachments/assets/562cd560-5fa2-4499-ab09-805e37d3453e)
