...
- 프로그램이 실제 실행되고 있는 상태의 능동적인 개체로, 운영체제의 스케줄링 및 관리 단위이다.
  - 프로그램(Program): 계획을 문서로 표현한 것으로, 표현 방법으로는 C, C++, Java 등이 있다. 
...
- 파일 읽기, 프로세스 동기화 등이 있다.
- 강제 종료 요청 시, 처리는 "Ctrl + C (강제 종료) = 시그널 체계"

**(상태 천이 vs 문맥 교환)**
- **상태 천이**: 프로세스의 상태가 바뀌는 것(예. Ready → Running)
  -  이벤트나 조건에 따라 프로세스의 논리적 상태가 변경된다.
- **문맥 교환**: CPU가 실행중인 프로세스를 다른 프로세스로 실행 대상을 바꾸는 작업
...
### ● → ▲이 되는 경우
- **Running → Waiting → Ready** (또 다른 예시 추가 예)
...
- 스케줄링은 Ready 상태의 프로세스들 중에서 하나를 선택하여 Running 상태로 결정하는 작업이 된다.
  ...
  - **Ready**: 실행 가능 상태로 스케줄러는 이 상태의 프로세스들 중 하나를 선택한다. 여기서 선택된 프로세스는 running 상태로 전환된다.(실행 준비는 됐는데, 순서만 안 온 상태)
...
- **Waiting과 Suspended의 차이**:
  - Waiting(대기 상태): 프로세스가 어떤 이벤트나 자원을 기다리는 상태로, 특정 조건이 반드시 충족되어야 다시 Ready 상태로 돌아갈 수 있다.
  - Suspended(중지 상태): 프로세스를 임시로 메모리에서 내린다.
    - 보통, 메모리 부족으로 OS가 해당 프로세스를 디스크에 내려놓는다.
    - 나중에 메모리에 여유가 생기면 다시 올려줄 수 있는데, 이 떄, 원래 상태(Ready 또는 Waiting)로 돌아간다.
**(각 상태의 의미 & 각 상태 간의 일어날 수 있는 경우의 예시)**
- 

## Linux 상태천이도
![image](https://github.com/user-attachments/assets/bb7b526a-557b-466f-8ed2-261509d27127)
- Runnable: 실행 가능한 상태(CPU 할당만 되면 바로 실행 가능)
- Running: CPU를 할당받아 실제로 실행 중인 상태
- Zombie: 프로세스가 종료되었지만, 부모 프로세스가 아직 종료 상태를 수거하지 않은 상태
- Interruptable (Waiting): I/O나 이벤트를 기다리는 일시 중단 상태로, Signal에 의해 꺠어날 수 있다.
- Uninterruptable (Waiting): 인터럽트나 Signal로도 깰 수 없는 대기 상태
- Stopped (Suspended): STOP signal 등에 의해 강제로 멈춰진 상태, Resume 하기 전까지는 실행 안된다.
- Traced (Suspended): 디버깅 도구에 의해 추적 중인 상태 (ptrace 등)
### 상태 전이 해석
- fork/clone → Runnable: 새로운 프로세스를 만들면 바로 실행 가능한 상태로 진입
- Runnable ↔ Running: CPU 스케줄링에 따라 번갈아 가며 전환
- Running → Zombie: 프로세스가 종료되었지만, 아직 부모가 wait() 호출을 안 함
- Interruptable/ Uninterruptable → Runnable: 기다리던 조건이 충족되면 다시 실행 가능 상태로 변경
- Running → Stopped: STOP signal로 인해 강제 중지
- Stopped → Runnable: CONT signal로 다시 재시작
- Running → Traced: 디버거가 프로세스를 추적할 때 진입
### 참고용
![image](https://github.com/user-attachments/assets/f977704b-b3c3-431a-b1d9-42c716380602)

## 프로세스 제어블록
![image](https://github.com/user-attachments/assets/a9d12bf2-33b7-42e3-8fa4-448099effdbf)
- 운영체제는 프로세스들에 대한 목록을 일괄적으로 기록하여 관리해야 한다.
- 스케줄링을 위해서는 프로세스들의 상태를 알 수 있어야 하고, ready 상태 프로세스들만의 목록을 가지고 있어야 하며, 이들 중에 하나를 선택하기 위해서는 선택의 기준이 되는 정보를 프로세스별로 가지고 있어야 한다.
- 따라서, 운영체제는 각 프로세스별로 기록해야 하는 정보를 일괄적으로 기록하여 관리할 필요가 있다.

### 간단한 프로세스 제어블록
![image](https://github.com/user-attachments/assets/ef7db609-830c-4434-b39f-fc091bfdae58)
- NextProcess: 프로세스 제어블록들을 리스트로 관리하기 위한 항목
- State: 프로세스의 상태
- Priority: 프로세스의 우선순위를 기록한다.
- ProcessId: 프로세스별로 가지는 고유번호
- UserId: 이 프로세스를 실행시킨 사용자의 고유번호
- 프로세스가 차지하고 있는 메모리 영역의 주소범위
  - 코드 영역: CodeAddr ~ CodeSize 크기
  - 데이터 영역: DataAddr ~ DataSize 크기
  - 스택 영역: StackAddr ~ StackSize 크기
- FileDescTable: 현재 open 상태의 파일들의 구조체에 대한 포인터들을 기록하는 배열
- Context: 문맥교환 과정에서 CPU 내부의 레지스터 값들을 기록하는 배열

### Ready 프로세스 목록
![image](https://github.com/user-attachments/assets/d6d09b9e-8087-4f24-9fe2-a615c1c042b2)
![image](https://github.com/user-attachments/assets/0690a20b-d1a9-41d2-940d-05ed7968047c)

## 문맥 & 문맥교환
![image](https://github.com/user-attachments/assets/60c7c9f1-7c6e-4661-8b88-003bc591cbfa)
- 운영체제는 필요할 때마다 스케줄링 함수를 실행하고 그 결과로 선택된 프로세스가 현재 실행중인 프로세스가 아니라면 문맥교환 절차를 처리해야 한다.
- **문맥(Context)**: 특정 시점의 프로세스의 현재 상태
- **문맥 교환(Context Switching)**: 운영체제가 실행할 프로세스를 다른 것으로 바꾸는 작업
  - 문맥 교환의 예. 한 권의 책을 읽다가 현재 읽던 위치 및 이야기의 흐름을 메모해 둔 후에 다른 책으로 바꿔서 읽기 시작하는 것이다. 나중에 다시 이 책을 계속해서 읽게 될 때, 이전의 메모로부터 읽던 위치를 찾고, 이야기의 흐름을 확인한 다음에 읽기를 계속하게 된다. 
